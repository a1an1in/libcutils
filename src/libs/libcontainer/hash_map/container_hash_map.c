...
iterator_t hash_map_iterator_init(iterator_t *it, struct hlist_node *hlist_node_p, uint32_t bucket_pos, struct hlist_head *hlist, container_t *ct);
iterator_t hash_map_iterator_next(iterator_t it);
int hash_map_iterator_equal(iterator_t it1,iterator_t it2);
...
int hash_map_init(container_t *ct)
{
...
iterator_t hash_map_begin(container_t *ct)
{
	return ct->begin;
}

iterator_t hash_map_end(container_t *ct)
{
	return ct->end;
}
int hash_map_insert(container_t *ct, void *data)
{
	...
	hash_func_fpt hash_func  = ct->priv.hash_map_priv.hash_func;
	uint32_t bucket_size     = ct->priv.hash_map_priv.bucket_size;;
	hash_map_pos_t *hash_map_begin_pos = &ct->begin.pos.hash_map_pos;

	dbg_str(DBG_CONTAINER_DETAIL,"hash_map_insert");

	mnode = (struct hash_map_node *)allocator_mem_alloc(ct->allocator,
			sizeof(struct hash_map_node) + data_size);
	if(mnode == NULL){
		dbg_str(DBG_CONTAINER_ERROR,"hash_map_insert,malloc err");
		return -1;
	}
	...
	if(hash_map_begin_pos->hlist_node_p == NULL || bucket_pos <= hash_map_begin_pos->bucket_pos){
		hash_map_iterator_init(&ct->begin, hlist[bucket_pos].first, bucket_pos, hlist, ct);
...
iterator_t hash_map_search(container_t *ct, void *key)
{
	iterator_t ret;
	struct hash_map_node *mnode;
	uint32_t key_size        = ct->priv.hash_map_priv.key_size;
	...
	dbg_str(DBG_CONTAINER_DETAIL,"hash_map_search");

	bucket_pos = hash_func(key,key_size,bucket_size); 
	assert(bucket_pos <= bucket_size);

	dbg_str(DBG_CONTAINER_DETAIL,"hash_map_search,bucket_pos=%d",bucket_pos);
	...
	hlist_for_each_safe(pos, next, &hlist[bucket_pos]){
		...
		if(!key_cmp_func(mnode->key,key,key_size)){
			...
			pthread_rwlock_unlock(&ct->head_lock);
			return hash_map_iterator_init(&ret, pos, bucket_pos, hlist, ct);
		}
	}
	pthread_rwlock_unlock(&ct->head_lock);
	if(mnode == NULL){
		return hash_map_iterator_init(&ret, NULL, bucket_pos, hlist, ct);
	}

	return ret;
}
int hash_map_delete(container_t *ct, iterator_t it)
{
	struct hash_map_node *mnode;
	...
	dbg_str(DBG_CONTAINER_IMPORTANT,"del hash_map ,bucket_pos:%d,cur node:%p,begin node:%p",
			...
			ct->begin.pos.hash_map_pos.hlist_node_p);
	pthread_rwlock_wrlock(&ct->head_lock);
	if(hash_map_iterator_equal(it,ct->begin)){
		dbg_str(DBG_CONTAINER_WARNNING,"del iter equal begain");
		next = hash_map_iterator_next(it);
...
int hash_map_destroy(container_t *ct)
{
	 dbg_str(DBG_CONTAINER_DETAIL,"hash_map_destroy");
	 iterator_t it;
	 for(	 it = hash_map_begin(ct);
			 !hash_map_iterator_equal(it,hash_map_end(ct));
			 it = hash_map_begin(ct))
	 {
		 dbg_str(DBG_CONTAINER_DETAIL,"destroy node:%p",it.pos.hash_map_pos.hlist_node_p);
		 hash_map_delete(ct,it);
	 }
	 if(hash_map_iterator_equal(ct->end,it)){
		 dbg_str(DBG_CONTAINER_WARNNING,"hash_map_destroy,hash_map is NULL");
		 allocator_mem_free(ct->allocator,ct->end.pos.hash_map_pos.hlist);
	 }
}

iterator_t hash_map_iterator_init(iterator_t *it,
...
iterator_t hash_map_iterator_next(iterator_t it)
{
	...
	if(hm_pos->hlist_node_p->next){
		...
		hash_map_iterator_init(&it, hlist_node_p, bucket_pos, hlist, ct);
		...
		hash_map_iterator_init(&it, NULL, bucket_size -1, hlist, ct);
	} else if(bucket_pos == bucket_size -1 ){
		dbg_str(DBG_CONTAINER_IMPORTANT,"container is null");
		hash_map_iterator_init(&it, NULL, bucket_pos, hlist, ct);
	}else{
		dbg_str(DBG_CONTAINER_ERROR,"hash_map_iterator_next err");
	}

	return it;
}
iterator_t hash_map_iterator_prev(iterator_t it)
{
	return it;
}
int hash_map_iterator_equal(iterator_t it1,iterator_t it2)
...
void *hash_map_iterator_get_pointer(iterator_t it)
{
	struct hash_map_node *mnode;
...
int container_hash_map_register(){
	container_module_t cm = {
		...
		.c_ops = {
			.init       = hash_map_init,
			.begin      = hash_map_begin,
			.end        = hash_map_end,
			.map_search = hash_map_search,
			.del        = hash_map_delete,
			.map_insert = hash_map_insert,
			.destroy    = hash_map_destroy,
		},
		.it_ops = {
			.next  = hash_map_iterator_next,
			.prev  = hash_map_iterator_prev,
			.equal = hash_map_iterator_equal,
			.iterator_get_pointer = hash_map_iterator_get_pointer
		}
	};
