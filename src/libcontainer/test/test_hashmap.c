...
int test_hash_map(void)
{
	...
	for(	it = container_begin(ct); 
			...
			it = iterator_next(it))
	{
		dbg_str(DBG_CONTAINER_DETAIL,"cur=%p next=%p",it.pos.hash_map_pos.hlist_node_p,it.pos.hash_map_pos.hlist_node_p->next);
	...
	it = container_map_search(ct, (void *)"22");
	if(it.pos.hash_map_pos.hlist_node_p){
		dbg_str(DBG_CONTAINER_DETAIL,"bocket pos:%d",it.pos.hash_map_pos.bucket_pos);
		mnode = container_of(it.pos.hash_map_pos.hlist_node_p,struct hash_map_node,hlist_node);
		dbg_buf(DBG_CONTAINER_DETAIL,"key:",mnode->key,mnode->value_pos);
		print_test((struct test *)iterator_get_pointer(it));
	}
